...
every channel and query across all networks, rendered exactly like a normal
chat buffer: nick colors, mentions, nick-column grouping, mIRC formatting and
timestamps all behave the same.
...
Open the buffer with =M-x clatter-unified=. It is created on the first captured
message, so it does not exist until something arrives.
