...
=*clatter-unified*= is a single chronological inbox showing live traffic from
...
| Variable                    | Default | Description                             |
|-----------------------------+---------+-----------------------------------------|
| =clatter-unified-enabled=     | =nil=     | Capture live messages into the inbox    |
| =clatter-unified-autoscroll=  | =t=       | Follow new messages when at the bottom  |
| =clatter-unified-hide-visible=  | =nil=     | Hide sources that currently have a window |
| =clatter-unified-hide-channels= | =nil=     | Hide these channel/query names (all nets) |

Set it before calling =clatter-setup=, which is what installs the capture hooks:

#+begin_src emacs-lisp
(setq clatter-unified-enabled t)
(clatter-setup)
#+end_src

Open the buffer with =M-x clatter-unified=. It is created on first use —
...
- =clatter-unified-hide-visible= collapses sources you are already looking at.
  =clatter-unified-hide-channels= is a denylist of channel/query names. Both can
  be on at once. The text stays in the buffer; changing either option brings it back.
...
- It is memory-bounded by =clatter-buffer-max-lines= like any other buffer, and
  it is invisible to activity tracking, so nothing is counted twice.
...
| =clatter-unified.el=       | Cross-network unified inbox buffer            | =clatter-unified-enabled= (off)       |
| =clatter-evil.el=          | Evil bindings for the special buffers         | automatic when evil is loaded         |
