...
| =clatter-track-shorten=          | =5=              | =nil=, an integer N, =drop-vowels= or =syllable= channel shortening |
...
If you place =clatter-track-mode-line-item= in a custom mode line (for
example a =doom-modeline= segment), clatter still appends it to the global
=mode-line-format=, but the indicator renders only once per window, so the
crumbs are never duplicated.
...
unique; query nicks and the server buffer are never shortened.  Hovering an
entry (in the mode line or =*clatter-activity*=) shows a tooltip with the full,
unshortened channel name and its unread count.
