...
| =clatter-display-on-join=        | =t=              | Show the channel buffer when you =/join= it from this session |
...
=clatter-display-on-join= keys off provenance, not timing: a channel is shown
only when you asked for it here with =/join=.  Joins Clatter did not request --
the hundreds a bouncer replays on reconnect, =:autojoin= channels, joins made
from another client, netsplit rejoins -- create their buffer silently and leave
your window layout alone.
